...
pub fn message_decryption(private_key: &[u8], encrypted_symmetric_key: &[u8], nonce: &[u8], encrypted_text: &[u8]) -> std::result::Result<(), CryptoError> {
    ...
    let private_key_corrected = kem.secret_key_from_bytes(&private_key).ok_or(CryptoError::InvalidInput)?;

    let encrypted_symmetric_key_corrected = kem.ciphertext_from_bytes(&encrypted_symmetric_key).ok_or(CryptoError::InvalidInput)?;
